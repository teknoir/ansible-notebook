...
description:
    - C(rsync) is a wrapper around rsync to make common tasks in your playbooks quick and easy.
...
options:
  src:
    description:
      - Path on the source host that will be rsyncd to the destination.
      - The path can be absolute or relative.
    type: str
    required: true
  dest:
    description:
      - Path on the destination host that will be rsyncd from the source.
      - The path can be absolute or relative.
    type: str
    required: true
  dest_port:
...
notes:
   ...
   - For the C(rsync) module, the "local host" is the host `the rsync task originates on`, and the "destination host" is the host
     `rsync is connecting to`.
   - The "local host" can be changed to a different host by using `delegate_to`.  This enables copying between two remote hosts or entirely on one
     remote machine.
   - >
     The user and permissions for the rsync `src` are those of the user running the Ansible task on the local host (or the remote_user for a
     delegate_to host when delegate_to is used).
   - The user and permissions for the rsync `dest` are those of the `remote_user` on the destination host or the `become_user` if `become=yes` is active.
   - In Ansible 2.0 a bug in the rsync module made become occur on the "local host".  This was fixed in Ansible 2.0.1.
   - Currently, rsync is limited to elevating permissions via passwordless sudo.  This is because rsync itself is connecting to the remote machine
     and rsync doesn't give us a way to pass sudo credentials in.
   - Currently there are only a few connection types which support rsync (ssh, paramiko, local, and docker) because a sync strategy has been
   ...
   - To exclude files and directories from being rsyncd, you may add C(.rsync-filter) files to the source directory.
   - rsync daemon must be up and running with correct permission when using rsync protocol in source or destination path.
   - The C(rsync) module enables `--delay-updates` by default to avoid leaving a destination in a broken in-between state if the underlying rsync process
...
- name: rsync two directories on one remote host.
  teknoir.rsync:
    src: /first/absolute/path
    dest: /second/absolute/path
  delegate_to: "{{ inventory_hostname }}"

- name: rsync and delete files in dest on the remote host that are not found in src of localhost.
  teknoir.rsync:
    src: some/relative/path
    dest: /some/absolute/path
    delete: yes
...
- name: rsync using an alternate rsync command
  teknoir.rsync:
    src: some/relative/path
    dest: /some/absolute/path
...
- name: rsync passing in extra rsync options
  teknoir.rsync:
    src: /tmp/helloworld
