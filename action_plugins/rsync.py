...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if delegate_to is None and self._connection.transport not in [
            'ssh', 'paramiko', 'local', 'teknoir'] + DOCKER + PODMAN + BUILDAH:
            result['failed'] = True
            result['msg'] = (
            ...
            if self._play_context.become and not rsync_path and self._remote_transport not in DOCKER + PODMAN:
                ...
                if self._play_context.become_method == 'sudo':
                    if self._play_context.become_user:
                        rsync_path = f'echo "{self._play_context.become_pass}" | sudo -S date >&/dev/null; sudo -u {self._play_context.become_user} rsync'
                    else:
                        rsync_path = f'echo "{self._play_context.become_pass}" | sudo -S date >&/dev/null; sudo rsync'
